  ...
  return (
    <Section id="features">
      <div className="container relative z-2">
        ...
        {selectedBenefit && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center bg-n-8/90 backdrop-blur-sm animate-in fade-in duration-300"
            onClick={closePopup}
          >
            <div
              className="relative max-w-2xl w-full mx-4 bg-gradient-to-br from-n-7 to-n-8 border border-n-6/50 rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-300"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Source Code Button */}
              <button
                onClick={handleSourceCode}
                className="absolute top-4 right-4 px-4 py-2 bg-n-6/80 hover:bg-n-5 text-n-1 rounded-lg transition-all duration-200 font-code text-xs font-semibold border border-n-5/50 hover:border-n-4"
              >
                Source Code
              </button>
